...
function getNextItemId() {
  try {
    Logger.log('🔍 getNextItemId() - START');
    
    var ss = getSpreadsheet();
    Logger.log('  ✓ Got spreadsheet');
    
    var sheet = ss.getSheetByName('Items');
    Logger.log('  ✓ Got Items sheet');
    ...
    if (lastRow <= 1) {
      Logger.log('  ℹ️ No items found (only header), returning IT001');
      return 'IT001';
    }
    
    var data = sheet.getDataRange().getValues();
    Logger.log('  📄 Data rows retrieved: ' + data.length);
    Logger.log('  🔍 Starting scan from row 2 (index 1) to row ' + data.length);
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      Logger.log('    Row ' + (i + 1) + ', Column A: "' + cellValue + '" (type: ' + typeof cellValue + ')');
      
      if (cellValue) {
        var itemId = cellValue.toString();
        Logger.log('      → String value: "' + itemId + '"');
        
        // Remove ALL non-numeric characters
        var cleaned = itemId.replace(/[^0-9]/g, '');
        Logger.log('      → After removing non-digits: "' + cleaned + '"');
        
        var numPart = parseInt(cleaned);
        Logger.log('      → parseInt result: ' + numPart + ' (isNaN: ' + isNaN(numPart) + ')');
        
        if (!isNaN(numPart)) {
          Logger.log('      → Comparing: ' + numPart + ' > ' + maxNum + ' ? ' + (numPart > maxNum));
          
          if (numPart > maxNum) {
            maxNum = numPart;
            Logger.log('      ✅ NEW MAX: ' + maxNum);
          }
        }
      } else {
        Logger.log('      ⚠️ Cell is empty/null');
      }
    }
    
    Logger.log('  📈 Final maxNum: ' + maxNum);
    
    var nextNum = maxNum + 1;
    Logger.log('  ➕ nextNum (maxNum + 1): ' + nextNum);
    
    // Format number with leading zeros (3 digits: 001, 002, 010, 100)
    var numStr = String(nextNum);
    Logger.log('  🔢 numStr before padding: "' + numStr + '" (length: ' + numStr.length + ')');
    
    while (numStr.length < 3) {
      numStr = '0' + numStr;
      Logger.log('    → Padding: "' + numStr + '"');
    }
    
    var nextId = 'IT' + numStr;
    Logger.log('  ✅ FINAL RESULT: ' + nextId);
...
function addItem(itemData) {
  try {
    ...
    Logger.log('  Data received: ' + JSON.stringify(itemData));
    ...
    var data = sheet.getDataRange().getValues();
    Logger.log('  🔍 Checking for duplicate Model Number...');
    
    for (var i = 1; i < data.length; i++) {
      if (data[i][1] && data[i][1].toString().toLowerCase() === itemData.modelNumber.trim().toLowerCase()) {
        Logger.log('  ❌ Model Number already exists: ' + itemData.modelNumber);
        return {success: false, message: 'Model Number "' + itemData.modelNumber + '" already exists'};
      }
    }
    
    Logger.log('  ✓ Model Number is unique');
    Logger.log('  🎯 Calling getNextItemId()...');
    
    // Generate next Item ID
    var itemId = getNextItemId();
    
    Logger.log('  🆔 Received Item ID: ' + itemId);
    
    // NEW: Check if Item ID already exists (should never happen, but safety check)
    var existingData = sheet.getDataRange().getValues();
    Logger.log('  🔍 Checking if Item ID ' + itemId + ' already exists...');
    
    for (var i = 1; i < existingData.length; i++) {
      if (existingData[i][0] && existingData[i][0].toString() === itemId) {
        Logger.log('  ❌❌❌ DUPLICATE ITEM ID FOUND: ' + itemId + ' at row ' + (i + 1));
        return {
          success: false, 
          message: 'Item ID ' + itemId + ' already exists in the system. Cannot save duplicate Item ID.'
        };
      }
    }
    
    Logger.log('  ✓ Item ID is unique, proceeding to save...');
    ...
    Logger.log('✅ addItem() - SUCCESS: ' + itemId + ' - ' + itemData.itemName);
...
/**
 * TEST FUNCTION - Run this directly to test getNextItemId
 */
function testGetNextItemId() {
  Logger.log('═══════════════════════════════════════');
  Logger.log('🧪 TEST: Running getNextItemId() directly');
  Logger.log('═══════════════════════════════════════');
  
  var result = getNextItemId();
...
/**
 * RESET ITEMS SHEET - Deletes all items and starts fresh
 * Run this ONCE to clean corrupted data
 * WARNING: This will delete all existing items!
 */
function resetItemsSheet() {
  try {
    Logger.log('🔧 Starting Items sheet reset...');
    ...
    if (lastRow <= 1) {
      Logger.log('ℹ️ Sheet is already empty (only header row)');
      return {success: true, message: 'Sheet is already empty'};
    }
    
    // Delete all data rows (keep header row 1)
    if (lastRow > 1) {
      sheet.deleteRows(2, lastRow - 1); // Delete from row 2 to last row
      Logger.log('✅ Deleted ' + (lastRow - 1) + ' rows');
    }
    
    Logger.log('✅ Items sheet reset complete - ready for fresh items');
    
    return {
      success: true,
      message: 'Items sheet reset successfully. All items deleted. Ready for fresh data.',
      deletedRows: lastRow - 1
    };
    
  } catch (error) {
    Logger.log('❌ Error in resetItemsSheet: ' + error);
    return {success: false, message: 'Error: ' + error.toString()};
  }
}
